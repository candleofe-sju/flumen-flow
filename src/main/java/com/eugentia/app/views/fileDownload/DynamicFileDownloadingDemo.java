 ...
 * @author mstahv
 */
@PageTitle("Dynamic File Downloading")
@Route(value = "dynamic-file-download", layout = MainLayout.class)
@RolesAllowed("USER")
public class DynamicFileDownloadingDemo extends VerticalLayout {

    private boolean cancelled = false;

    private DynamicFileDownloader actaulButtonLikeDownloadButton;
    ...
    public DynamicFileDownloadingDemo() {
        ...
        disableOnClick.addDownloadFinishedListener(e -> {
            new Thread(() -> {
                ...
                disableOnClick.getUI().ifPresent(ui1 -> {
                    ui1.access(() -> disableOnClick.setEnabled(true));
                });
            }).start();
        });
        add(disableOnClick);
        ...
        b.addClickListener(e -> cancelled = true);
        add(interruptable, b);
